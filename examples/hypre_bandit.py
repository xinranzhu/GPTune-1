...
def objectives(point):
    ...
    runtime = hypredriver(params, niter=1, JOBID=JOBID, budget=budget)
...
def main(): 
    ...
    if(TUNER_NAME=='hpbandster_bandit'):
        NI = ntask
        NS = nruns
        (data,stats)=callhpbandster_bandit.HpBandSter(T=giventask, NS=NS, tp=problem, computer=computer, run_id="hpbandster_bandit", niter=1)
        print("stats: ", stats)
        """ Print all input and parameter samples """
        for tid in range(NI):
            print("tid: %d" % (tid))
