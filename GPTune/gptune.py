...
class GPTune(object):
    ...
    def __init__(self, tuningproblem: TuningProblem, computer: Computer = None, data: Data = None, options: Options = None, driverabspath=None, models_update=None, **kwargs):
        ...
        self.problem = Problem(
            tuningproblem, driverabspath=driverabspath, models_update=models_update)
        if (computer is None):
            computer = Computer()
    ...
    def MLA(self, NS, NS1=None, NI=None, Igiven=None, **kwargs):
        ...
        if (self.data.P is not None and len(self.data.P[0]) >= NS and self.data.O is not None):
        ...
        # building the MLA model for each of the given tasks
 ...
 # normalize the data as the user always work in the original space
        ...
        # from a list of (list of lists) to a list of 2D numpy arrays
 ...
 #        if (self.mpi_rank == 0):
        ...
        if (self.data.I is None):
            ...
            if (NI is None):
                raise Exception(
                    "Number of problems to be generated (NI) is not defined")

            check_constraints = functools.partial(
                self.computer.evaluate_constraints, self.problem, inputs_only=True, kwargs=kwargs)
            self.data.I = sampler.sample_inputs(
                n_samples=NI, IS=self.problem.IS, check_constraints=check_constraints, **kwargs)
        ...
        if (self.data.P is None):
            ...
            if (NS1 is None):
                # General heuristic rule in the litterature
                NS1 = min(NS - 1, 3 * self.problem.DP)

            check_constraints = functools.partial(
                self.computer.evaluate_constraints, self.problem, inputs_only=False, kwargs=kwargs)
            self.data.P = sampler.sample_parameters(
                n_samples=NS1, I=self.data.I, IS=self.problem.IS, PS=self.problem.PS, check_constraints=check_constraints, **kwargs)
 ...
 #                    x = np.concatenate(x, np.array([m(x) for m in self.problems.models]))
        ...
        if (self.data.O is None):
            self.data.O = self.computer.evaluate_objective(
                self.problem, self.data.I, self.data.P, self.data.D, options=kwargs)
        t2 = time.time_ns()
 ...
 #            self.data = mpi_comm.bcast(None, root=0)
        ...
        modelers = [eval(
            f'{kwargs["model_class"]} (problem = self.problem, computer = self.computer)')]*self.problem.DO
        searcher = eval(
            f'{kwargs["search_class"]}(problem = self.problem, computer = self.computer)')
        optiter = 0
        # YL: each iteration adds 1 (if single objective) or at most kwargs["search_more_samples"] (if multi-objective) sample until total #sample reaches NS
        while len(self.data.P[0]) < NS:
            ...
            if(self.problem.models_update is not None):
                ...
                # from a collection of 2D numpy arrays to a list of (list of lists)
            ...
            for o in range(self.problem.DO):
                tmpdata = copy.deepcopy(self.data)
                tmpdata.O = [copy.deepcopy(self.data.O[i][:, o].reshape(
                    (-1, 1))) for i in range(len(self.data.I))]
                if(self.problem.models is not None):
                    for i in range(len(tmpdata.P)):
                        points0 = tmpdata.D[i]
                        t = tmpdata.I[i]
                        I_orig = self.problem.IS.inverse_transform(
                            np.array(t, ndmin=2))[0]
                        points1 = {self.problem.IS[k].name: I_orig[k]
                                   for k in range(self.problem.DI)}
                        modeldata = []
                        for p in range(len(tmpdata.P[i])):
                            x = tmpdata.P[i][p]
                            x_orig = self.problem.PS.inverse_transform(
                                np.array(x, ndmin=2))[0]
                            points = {self.problem.PS[k].name: x_orig[k]
                        ...
                        # YL: here tmpdata in the normalized space, but modeldata is the in the original space
                        tmpdata.P[i] = np.hstack((tmpdata.P[i], modeldata))
                # print(tmpdata.P[0])
            ...
            t1 = time.time_ns()
            res = searcher.search_multitask(
                data=self.data, models=modelers, **kwargs)

            # YL: this makes sure P has the same length across all tasks
            more_samples = NS-len(self.data.P[0])
            for x in res:
    ...
    #            if (self.mpi_rank == 0):
            ...
            t1 = time.time_ns()
            newdata.O = self.computer.evaluate_objective(
                problem=self.problem, I=newdata.I, P=newdata.P, D=newdata.D, options=kwargs)
            t2 = time.time_ns()
            time_fun = time_fun + (t2-t1)/1e9
    #                if ((self.mpi_comm is not None) and (self.mpi_size > 1)):
 ...
 # denormalize the data as the user always work in the original space
    ...
    def TLA1(self, Tnew, NS):
        ...
        INorms = np.vstack([INorms[i] for i in range(ntso)]
                           ).reshape((ntso, self.problem.DI))

        tmp = []
        for t in Tnew:
            INorm = self.problem.IS.transform(np.array(t, ndmin=2))[0]
            tmp.append(INorm.reshape((-1, self.problem.DI)))
        InewNorms = np.vstack([tmp[i] for i in range(ntsn)]).reshape(
            (ntsn, self.problem.DI))
        ...
        for k in range(self.problem.DP):
            ...
            M.optimize_restarts(num_restarts=kwargs['model_restarts'], robust=True, verbose=kwargs['verbose'], parallel=(kwargs['model_threads'] > 1), num_processes=kwargs['model_threads'],
                                messages=kwargs['verbose'], optimizer='lbfgs', start=None, max_iters=kwargs['model_max_iters'], ipython_notebook=False, clear_after_finish=True)
            MSopt.append(M)

        aprxoptsNorm = np.hstack([MSopt[k].predict_noiseless(InewNorms)[0] for k in range(
            self.problem.DP)])  # the index [0] is the mean value, [1] is the variance
        aprxoptsNorm = np.minimum(
            aprxoptsNorm, (1-1e-12)*np.ones((ntsn, self.problem.DP)))
        aprxoptsNorm = np.maximum(
            aprxoptsNorm, (1e-12)*np.ones((ntsn, self.problem.DP)))
        ...
        # TnewNormList=[]
        for i in range(ntsn):
            # this makes sure for each task, there is only one sample parameter set
            aprxoptsNormList.append([aprxoptsNorm[i, :]])
            # InewNormList.append(InewNorms[i,:])

        t1 = time.time_ns()
        O = self.computer.evaluate_objective(
            problem=self.problem, I=InewNorms, P=aprxoptsNormList, options=kwargs)
        t2 = time.time_ns()
        time_fun = time_fun + (t2-t1)/1e9
...
class GPTune_MB(object):
    ...
    def __init__(self, tp: TuningProblem, computer: Computer = None, options: Options = None, **kwargs):
        ...
        smax = int(np.floor(np.log10(
            options['budget_max']/options['budget_min'])/np.log10(options['budget_base'])))
        self.budgets = [options['budget_max'] /
                        options['budget_base']**x for x in range(smax+1)]
        print(f'smax = {smax}, budgets = {self.budgets}')
    ...
    def MB_LCM(self, NS=None, Igiven=None, **kwargs):
        ...
        for s in range(len(self.budgets)):  # loop over the budget levels
            ...
            ns = self.NSs[s]
            newtasks = []
            # print(f'Current bracket, s = {s}, budget = {budget}, ns = {ns}')
        ...
        for s in range(1, len(self.budgets)):
            ...
            while budget < self.options['budget_max']:
                idx_sort = list(map(np.argsort, temp_O))
                temp_O = [y[x[:ratio]] for (x, y) in zip(idx_sort, temp_O)]
                temp_P = [y[x[:ratio]] for (x, y) in zip(idx_sort, temp_P)]
